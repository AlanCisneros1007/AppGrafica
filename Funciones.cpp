			...
			if (registros[i].clavemaquina == 0 && i < 50) {
				...
				if (clavemaquina != 0) {
					...
					if (strlen(ncp) < 11) {
						...
						cout << "El usuario con matricula [" << i << "] tiene un numero que excede los 10 digitos. "<<endl;
						strcpy_s(nc, "error");
					}
					archivo >> ce;
				}
...
void registrarMaquinas(int noUsu) {
	...
	char nm[30] = ""; char np[30] = ""; char nc[11] = ""; char ce[40] = "",nombretxt[40] = "",ncp[100] ="",ncc[100]="";
	...
	while (i < noUsu){
		...
		cout << "\n";
		if (validarCorreo(ce,0) && validarRangoClave(clavemaquina) && validarCadena(nm) && validarCadena(np) == 1 && yaExiste == false && clavemaquina != 0 && validarTelefono(nc)==1 && nc!="error" && ce!="error") {
			archivo << clavemaquina << " ";
			archivo << nm << " ";
			archivo << np << " ";
			archivo << nc << " ";
...
void getMatriculas(int* hola) {
	for (int i = 0; i < 50; i++) {
			hola[i] = registros[i].clavemaquina;
	//		nombres[i] = registros[i].nombremaquina;
